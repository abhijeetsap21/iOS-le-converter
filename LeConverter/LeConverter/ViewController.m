...
- (void)viewDidLoad {
   ...
   //This creates object of the brain instance
    _convertObject = [[brain alloc] init];
    
   // Populating the arrays
    selected = @[@"Sq Kms",@"Sq Meters",@"Sq Foot"];
    scales = @[@"Area",@"Length",@"temperature"];
    areaunits = @[@"Sq Kms",@"Sq Meters",@"Sq Foot"];
    lengthunits = @[@"Meter",@"Kms",@"Mile",@"Foot"];
...
- (BOOL) textFieldShouldReturn:(UITextField *)inputMeasurement{
    ...
    //Return value function for UI text field
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component{
    ...
    self.labelresult.text = [NSString stringWithFormat:@"%f",res];
